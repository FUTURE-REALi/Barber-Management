...
const CancelledBookings = ({ bookings, loading }) => {
  if (loading) return <div className="text-center text-gray-500">Loading...</div>;
  if (!bookings.length) return <div className="text-center text-gray-500">No cancelled bookings.</div>;
  return (
      ...
      {bookings.map((b) => (
        <div key={b._id} className="border rounded-lg p-4 flex flex-col md:flex-row md:items-center justify-between bg-white shadow">
          <div>
            ...
            <div className="text-gray-500 text-sm">Status: <span className="text-red-500">Cancelled</span></div>
